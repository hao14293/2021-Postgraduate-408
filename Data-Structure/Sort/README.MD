...
* 时间复杂度：一共操作了 n-1 趟，每趟都分为比较和移动两部分。最好情况，只比较不移动，O(n);最坏情况，比较 n<sup>2</sup>/2 次，移动 n<sup>2</sup>/2 次， 时间复杂度为O(n^2).
...
![quickSort.gif](https://i.loli.net/2018/11/18/5bf167ec2b1b6.gif)
